...
{
    ARRT::const_iterator it = arr.begin();
    ARRT::const_iterator end = arr.end();

    pstream->write_dword(arr.size());
    for(;it!=end;++it)
    {
        const ARRT::value_type* pp = &*it;
        pp->save(pstream);
    }
}
...
template< typename ARRT >
size_t read_array(const stream* pstream, ARRT& arr, size_t (*loader_fp)(const stream*, typename ARRT&))
{
    size_t counter = 0;
